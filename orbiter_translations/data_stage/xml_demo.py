...
def basic_task_filter(val: dict) -> list | None:
    ...
    if isinstance(val, dict):
        ...
        try:
            return jq.compile(""".Record[] | select(.["@Type"] == "CustomStage")""").input_value(val).all()
...
def task_common_args(val: dict) -> dict:
    ...
    try:
        task_id: str = (
            jq.compile(""".Property[] | select(.["@Name"] == "Name") | .["#text"]""").input_value(val).first()
        )
    except ValueError:
        task_id = "UNKNOWN"
...
def extract_sql_statements(root: dict) -> list[str]:
    ...
    if root:
        return [
            sql.strip()
            for tag in ["BeforeSQL", "SelectStatement", "AfterSQL"]
...
def sql_command_rule(val) -> OrbiterSQLExecuteQueryOperator | None:
    ...
    try:
        if sql := jq.first(
            """.Collection[] | .SubRecord[] | .Property[] | select(.["@PreFormatted"] == "1") | .["#text"]""", val
        ):
