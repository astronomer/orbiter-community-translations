...
def test_oozie_demo(project_root):
    actual = translation_ruleset.translate_fn(translation_ruleset, (project_root / "tests/oozie/demo/workflow/"))
    ...
    assert sorted(list(list(actual.dags.values())[0].tasks["hdfs_node"].downstream)) == sorted(["end", "fail"])
