...
def test_control_m_demo(project_root):
    actual = translation_ruleset.translate_fn(translation_ruleset, (project_root / "tests/control_m/demo/workflow/"))
    ...
    assert sorted(list(list(actual.dags.values())[0].tasks["task_a"].downstream)) == sorted(["task_b"])
