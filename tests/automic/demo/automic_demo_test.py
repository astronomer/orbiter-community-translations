...
def test_automic_demo(project_root):
    actual = translation_ruleset.translate_fn(translation_ruleset, (project_root / "tests/automic/demo/workflow/"))
    ...
    assert sorted(list(list(actual.dags.values())[0].tasks["insert_into_database"].downstream)) == sorted(["end"])
