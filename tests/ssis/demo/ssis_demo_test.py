...
def test_ssis_demo(project_root):
    actual = translation_ruleset.translate_fn(translation_ruleset, (project_root / "tests/ssis/demo/workflow/"))
    ...
    assert sorted(list(list(actual.dags.values())[0].tasks["lookup_date_key"].downstream)) == sorted(
        ["sample_ole__db_destination"]
    )
